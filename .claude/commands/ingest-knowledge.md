...
```json
{
  ...
  "content_markdown": "{full content as markdown}"
}
```
...
### Step 6: Generate Embeddings (if LanceDB available)

If LanceDB MCP is configured:
1. Generate embedding for the summary + key points
2. Store in `aiconfig_embeddings` table with metadata
3. Enable semantic search for this knowledge
...
/ingest-knowledge https://martinfowler.com/articles/microservices.html
```
